...
export async function GET(req: Request): Promise<NextResponse> {
  try {
    ...
    if (!code) {
      return errorResponse("ln_err", "Failed to log in with LinkedIn");
    }

    let url = `${process.env.NEXT_PUBLIC_API_URL_IDN || "https://identity-api.sandbox.pepagora.org/"}auth/oauthVerification`;
