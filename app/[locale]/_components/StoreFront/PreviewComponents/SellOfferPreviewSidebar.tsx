...
import { OfferType, SellOfferPreviewData } from "@/app/[locale]/_interface/SellOfferInterface";
...
interface PreviewSidebarProps {
  ...
  previewData: SellOfferPreviewData | null;
}
const SellOfferPreviewSidebar: FC<PreviewSidebarProps> = ({
        ...
        content={() => (
          <>
            <div className="three-column-layout">
                    ...
                    <div className="product-info-block">
                      ...
                      {/* Default */}
                      {
                        ...
                        <div className='product-info-group'>
                          <div className='offer-price-block'>
                            <Typography variant='span' className='o-p-b-strike'>{t("pricePlaceHolder")}</Typography>
                            <span className='badge-comp b-c-sm offer-price-badge'>-- %</span>
                          </div>
                          <Typography variant='span' className='offer-price-value'>{t("rangePlaceHolder")} <span className='o-p-v-pc'>/ pc</span></Typography>
                        </div>
                      }
                      {/* Pricing Fixed */}
                            ...
                            <div className="product-info-group">
                              ...
                              <Typography variant="span" className="offer-price-value">
                                {offerInfo?.currency?.symbol
                                  ? offerInfo?.discountPercent
                                    ? `${offerInfo?.currency?.symbol} ${offerInfo?.pricing?.unitPrice
                                    }`
                                    : `${productDetails?.currency?.symbol} ${productDetails?.pricing?.unitPrice
                                    }`
                                  : "--"}
                            ...
                            <div className="product-info-group">
                              ...
                              {offerInfo?.discountPercent && (
                                <div className="offer-price-block">
                                  <Typography variant="span" className="o-p-b-strike">
                                    {productDetails?.currency?.symbol
                                      ? `${productDetails?.currency?.symbol} ${productDetails?.pricing?.minPrice
                                      } - ${productDetails?.currency?.symbol} ${productDetails?.pricing?.maxPrice}`
                                      : "--"}
                                  </Typography>
                                  <span className="badge-comp b-c-sm offer-price-badge">
                                    {offerInfo?.discountPercent} %
                              ...
                              <Typography variant="span" className="offer-price-value">
                                {offerInfo?.currency?.symbol
                                  ? offerInfo?.discountPercent
                                    ? `${offerInfo?.currency?.symbol} ${offerInfo?.pricing?.minPrice
                                    } - ${offerInfo?.currency?.symbol} ${offerInfo?.pricing?.maxPrice
                                    }`
                                    : `${productDetails?.currency?.symbol} ${productDetails?.pricing?.minPrice
                                    } - ${productDetails?.currency?.symbol} ${productDetails?.pricing?.maxPrice
                                    }`
                              ...
                              <table className="variants-table">
                                <thead>
                                  <tr>
                                    ...
                                    <th>{t("qtyTo")}</th>
                                    <th>{t("price")}</th>
                                    {offerInfo.discountPercent && <th>{t("discountedPrice")}</th>}
                                  </tr>
                                </thead>
                                <tbody>
                                  {
                                    offerInfo?.pricing?.bulkPrices.map((price, i) => {
                                      ...
                                      return (
                                        <tr key={i}>
                                          ...
                                          <td>{
                                            offerInfo.discountPercent ?
                                              `${productDetails?.currency?.symbol}${getOriginalPrice(price?.price, offerInfo?.discountPercent as number)}` :
                                              `${productDetails?.currency?.symbol}${price?.price}`
                                          }</td>
                                          {offerInfo.discountPercent && <td>
                                            {`${productDetails?.currency?.symbol}${price?.price}`}
                                          </td>}
                                        </tr>
                                      )
                      ...
                      <div className="product-info-group">
                        ...
                        {productDetails?.category &&
                          ...
                          >
                            {`${productDetails.category.name} >  ${productDetails.subCategory.name} ${productDetails.productCategory ? `> ${productDetails.productCategory?.name}` : ""}`}
                          </Typography> :
                          <Typography
                            variant="span"
                            className="light-label"
                          >
                            {`${t("category")} >  ${t("subCategory")} > ${t("productCategory")}`}
                          </Typography>}
                      </div>
                    </div>
                  </div>
              ...
              <ButtonIconRight name={t("postSellOffer")}>
                <RightArrowIcon />
              </ButtonIconRight>
...
export default SellOfferPreviewSidebar;
